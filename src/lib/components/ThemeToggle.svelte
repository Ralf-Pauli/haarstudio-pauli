<script lang="ts">
    ...
    onMount(() => {
        const input: HTMLInputElement = document.getElementById("toggle-switch-cb") as HTMLInputElement;
        if (!input) return;
        input.checked = localStorage.theme !== "dark";
...
<div class="toggle-switch-container relative w-[24px]">
    ...
    <label for="toggle-switch-cb" class="absolute top-0 left-0 svg-overlay">
        <svg xmlns="http://www.w3.org/2000/svg" width="1.3em" height="1.3em" viewBox="0 0 24 24" class="night-icon"
             {...$$props}
        >
            <path
                    fill="currentColor"
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="1.3em" height="1.3em" viewBox="0 0 24 24" class="day-icon"
             {...$$props}
        >
            <path
                    fill="currentColor"
...
<style>
    ...
    #toggle-switch-cb:checked + .svg-overlay .day-icon {
        opacity: 1;
    }

    #toggle-switch-cb:checked + .svg-overlay .night-icon {
        opacity: 0;
    }
</style>
